...
@Component({
  ...
  template: `
    <nav class="flex items-center justify-between h-16 bg-white border-gray-300 border-b-1 shadow-md p-5">
      <ep-text class="text-gray-500" *ngIf="title$ | async as title">{{title | translate}}</ep-text>
      <img src="/assets/icons/exit.svg" alt="Sign Out"
           class="cursor-pointer text-red-500 hover:scale-125 transition-transform h-5 w-5"
           (click)="signOut()"/>
    </nav> `,
