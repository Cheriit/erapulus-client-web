...
@Component({
  selector: 'ep-student-users-list',
  template: `
    <ep-container>
      <ep-header [headerType]="headerSize.H3">{{'management-panel.user.list.student.header' | translate}}</ep-header>
...
export class StudentUserListComponent implements OnInit, OnDestroy {
  ...
  private updateRoute (): void {
    this.location.go(`${this.router.url.split('?')[0]}?student_page=${this.tableConfiguration.currentPage}&student_name=${this.tableConfiguration.filters.value['name']}`);
  }
